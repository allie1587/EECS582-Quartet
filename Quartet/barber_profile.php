...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (isset($_FILES['photo_image']) && $_FILES['photo_image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['photo_image']['tmp_name'], $file_path)) {
            ...
            $stmt = $conn->prepare("UPDATE Barber_Information SET Photo=? WHERE Username=?");
    ...
    if (isset($_POST['working_days'])) {
        ...
        try {
            ...
            $delete_stmt = $conn->prepare("DELETE FROM Barber_Availability WHERE barber_id = ?");
            ...
            foreach ($days as $day) {
                if (isset($_POST['working_days'][$day]) && $_POST['working_days'][$day] == 'on') {
                    $start = $conn->real_escape_string($_POST['start_time'][$day]);
                    $end = $conn->real_escape_string($_POST['end_time'][$day]);

                    if (!empty($start) && !empty($end)) {
                        $insert_stmt = $conn->prepare("INSERT INTO Barber_Availability 
                            (barber_id, day_of_week, start_time, end_time) 
                            VALUES (?, ?, ?, ?)");
                        $insert_stmt->bind_param("ssss", $username, $day, $start, $end);
                        $insert_stmt->execute();
                        $insert_stmt->close();
                    }
                }
...
if (isset($_SESSION['username'])) {
    ...
    // Get barber info
    $stmt = $conn->prepare("SELECT * FROM Barber_Information WHERE Username = ?");
    if ($stmt) {
    ...
    // Get availability
    $stmt = $conn->prepare("SELECT * FROM Barber_Availability WHERE barber_id = ?");
    if ($stmt) {
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $result = $stmt->get_result();
...
<body>
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data" id="barber-profile-form" onsubmit="return validateBeforeSubmit();">
            ...
            <?php
            // Fetch barber's availability from the database
            $availability = [];
            $stmt = $conn->prepare("SELECT day_of_week AS day, start_time AS start, end_time AS end FROM Barber_Availability WHERE barber_id = ?");
            $stmt->bind_param("s", $username);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            <div class="form-section">
                ...
                $days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
                foreach ($days as $day):
                    $day_availability = array_filter($availability, fn($item) => $item['day'] === $day);
                    $day_availability = reset($day_availability);
                ?>
                    <div class="day-row">
                        <div class="day-checkbox">
                            <input type="checkbox" id="<?= strtolower($day) ?>" name="working_days[<?= $day ?>]"
                                <?= $day_availability ? 'checked' : '' ?>>
                            <label for="<?= strtolower($day) ?>"><?= $day ?></label>
                        </div>
                        <div class="time-inputs">
                            <input type="time" name="start_time[<?= $day ?>]"
                                value="<?= $day_availability['start'] ?? '' ?>"
                                <?= !$day_availability ? 'disabled' : '' ?> required>
                            to
                            <input type="time" name="end_time[<?= $day ?>]"
                                value="<?= $day_availability['end'] ?? '' ?>"
                                <?= !$day_availability ? 'disabled' : '' ?> required>
                        </div>
                    </div>
                <?php endforeach; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Enable/disable time inputs when checkbox is toggled
            document.querySelectorAll('.day-checkbox input[type="checkbox"]').forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    const dayRow = this.closest('.day-row');
                    const timeInputs = dayRow.querySelectorAll('.time-inputs input');
